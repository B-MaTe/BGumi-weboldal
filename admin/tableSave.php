<?php
    ...
    if (isset($_POST['id'])) {
        ...
        if ($id == 0) {
            $insert = "INSERT INTO munkaink (name, sku, description, date) VALUES ('$name', '$sku', '$desc', '$date')";
            $res = mysqli_query($link, $insert);

            $id = mysqli_insert_id($link);
        } else {
            $update = "UPDATE munkaink SET name='" . $name . "', sku='" . $sku . "', description='" . $desc . "', date='" . $date . "' WHERE id=" . $id;
